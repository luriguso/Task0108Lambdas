...
import level1.exercise2.classData.Name;
...
public class NamesManagement {
    ...
    public void addNamesInitial(){
        ...
        names.add(new Name("Ricardo"));
        names.add(new Name("Jane"));
        names.add(new Name("Julie"));
        names.add(new Name("Mauricio"));
        names.add(new Name("Mary"));
    }

    public void select(){
        this.names.stream()
                .filter(word->word.getName().toLowerCase().contains("o") && word.getName().length()>5)
                .map(Name::getName )
